...
# def subtract_schemes(scheme1, scheme2, tmp_dir):
#     logging.info("Subtracting datasets with %d and %d entries", len(scheme1("rhoa")), len(scheme2("rhoa")))
#     # create global electrode register
#     logging.info("Creating electrode register...")
#     electrodes = []
#     scheme1_electrodes = scheme1.sensorPositions()
#     for i in range(len(scheme1_electrodes)):
#         electrodes.append(scheme1_electrodes[i])
...
#
#     # create electrode dictionaries
#     logging.info("Creating electrode dictionaries...")
#     scheme1_dic = {-1: -1, 0: 0}
#     for i in range(len(scheme1_electrodes)):
#         scheme1_dic.update({i + 1: i + 1})
#     scheme2_dic = {-1: -1, 0: 0}
#     for i in range(len(scheme2_electrodes)):
#         for j in range(len(electrodes)):
#             if scheme2_electrodes[i] == electrodes[j]:
#                 scheme2_dic.update({i + 1: j + 1})
#
#     # subtract configurations
#     scheme_sensors = []
#     scheme_data = []
...
#         ])
#     for i in range(len(scheme2("rhoa"))):
#         idx = scheme_sensors.index(
#             scheme1_dic[scheme2("a")[i]],
#             scheme1_dic[scheme2("b")[i]],
#             scheme1_dic[scheme2("m")[i]],
#             scheme1_dic[scheme2("n")[i]]
#         )
#         del scheme_sensors[idx]
#         del scheme_data[idx]
#         logging.info("Subtracting complete (%d entries)", len(scheme_data))
#
#         # write to file
#         logging.info("Writing scheme to file...")
#         tmp_file = tmp_dir + "tmpMerge.shm"
#         f = open(tmp_file, "w")
#         # - electrodes
#         f.write("%d\n" % len(electrodes))
